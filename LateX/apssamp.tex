...
\vspace{-3mm}
\noindent The classification of transient astronomical objects from time-series photometric data presents a significant challenge for modern large-scale surveys. This study presents a bidirectional Long Short-Term Memory (LSTM) neural network for classifying light curves from the Photometric LSST Astronomical Time-series Classification Challenge (PLAsTiCC) dataset. The original fourteen object classes were reorganized into five generalized categories (S-Like, Fast, Long, Periodic, and Non-Periodic) to address class imbalance. Data preprocessing included sequence padding, temporal rescaling, and min-max flux normalization. The network architecture incorporated masking layers to handle variable-length sequences and GlobalMaxPooling for dimensionality reduction. Model evaluation was performed using ROC curves, Precision-Recall curves, and confusion matrices on a test set of 19,920 objects. The model demonstrated strong performance for S-Like and Periodic classes, achieving ROC area under the curve (AUC) values of 0.95 and 0.99, and Precision-Recall AUC values of 0.98 and 0.89, respectively. However, performance was significantly lower for Fast and Long classes (ROC AUC of 0.68 for Long class), and the model exhibited difficulty distinguishing between Periodic and Non-Periodic objects. Evaluation on partial light curve data (5, 10, and 20 days from detection) revealed substantial performance degradation, with increased misclassification toward the S-Like class. These findings indicate that class imbalance and limited temporal information in early observations are primary limitations. Future improvements should focus on class balancing strategies and preprocessing techniques that emphasize detection moments to enhance classification of minority classes and early-time predictions.
\end{abstract}
